...
The teams are selected to show contrasting paths rather than to represent the league as a whole. Detroit supplies a consistently strong benchmark; Tampa Bay and Nashville show different rises; and the Islanders, Penguins, and Blackhawks make the reversals easiest to see.
...
This chart gives a quick overview, but it asks the colorbar to do almost all of the work. Exact values require estimation, the missing season competes visually with the data, and the two post-lockout rebuilds are easy to overlook.

For the final version, each data value becomes a \code{Circle} artist. Here we deliberately encode points percentage twice. The radius follows the original expanding-bubble rule, \code{0.25 + value / 2}, so stronger seasons swell into their neighbors. Every circle uses Matplotlib's first default color, \code{C0}, while its opacity follows the same value; direct labels provide exact values. The redundant encodings create a rhythm across each row rather than asking a legend to do all the work. Compact horizontal season labels keep the top of the grid quiet.

The text labels also become artists. Each team's mark is loaded with Pillow, placed in an \code{OffsetImage}, and anchored beside its row with an \code{AnnotationBbox}. A shaded band reserves space for the cancelled season, where the letters in ``LOCKOUT'' are stacked downward but kept upright. Finally, two annotations bring the Penguins' and Blackhawks' changes forward without erasing the other teams that provide context.
...
\pyfilelines{hockey-heat.py}{42}{105}

The image-label loop uses the same artist pattern once per team.

\pyfilelines{hockey-heat.py}{130}{140}

\begin{center}
    \includegraphics[width = \textwidth]{figures/poetryplots/hockey-heat.pdf}
\end{center}

The finished figure now makes a specific claim. Pittsburgh fell to a 35 percent points percentage immediately before and after the lockout, then reached 64 percent in 2006--07. Chicago's recovery took longer, moving from 36 percent in 2003--04 to 63 percent in 2008--09. Detroit's consistently large, dark circles on both sides of the lockout keep those improvements in perspective: a rebound and sustained excellence are different stories.

The custom treatment earns its complexity when a small matrix contains a few values worth reading and a pattern worth annotating. For a much larger matrix, the direct labels and circles would become clutter; the simpler \code{imshow()} version would usually be the better choice.
