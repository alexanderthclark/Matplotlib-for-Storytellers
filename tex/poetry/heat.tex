...
Before placement, the helper crops transparent padding and fits every mark inside the same 170-by-170-pixel box; applying one shared zoom then makes the six labels roughly uniform. Treat that as a starting point: Washington's wide wordmark may deserve a slightly larger team-specific zoom so the small ``capitals'' lettering remains legible, whereas the other five marks contain no text.
...
\pyfilelines{hockey-heat.py}{45}{118}

The image-label loop uses the same artist pattern once per team.

\pyfilelines{hockey-heat.py}{119}{129}
