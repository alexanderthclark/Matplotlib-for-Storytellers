...
For the final version, each data value becomes a \code{Circle} artist. Here we deliberately encode points percentage twice. The radius follows the original expanding-bubble rule, \code{0.25 + value / 2}, so stronger seasons swell into their neighbors; color reinforces the same pattern, while direct labels provide exact values. The redundant encodings create a rhythm across each row rather than asking a legend to do all the work.
...
\pyfilelines{hockey-heat.py}{43}{116}

The image-label loop uses the same artist pattern once per team.

\pyfilelines{hockey-heat.py}{141}{151}
