...
The previous examples used \code{imshow()} to turn a rectangular array into a heatmap. Now we will keep the grid, but draw each observation ourselves. Doing so gives us control over the treatment of missing data, the labels inside the cells, and the story-telling annotations.
...
First, here is the conventional version. We pivot the data into a team-by-season matrix and pass it to \code{imshow()}. A masked value and a modified ``bad'' color make the lockout visibly different from a low-performing season.
