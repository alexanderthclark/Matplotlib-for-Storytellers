...
For the final version, each data value becomes a \code{Circle} artist. The circles all have the same size, so area does not introduce a second quantitative encoding. Color carries the points percentage, while direct labels provide exact values. A shaded band explicitly reserves space for the cancelled season. Finally, two annotations bring the Penguins' and Blackhawks' changes forward without erasing the other teams that provide context.

The complete script also contains the layout, title, colorbar, source note, and export commands. The custom artist loop and the story annotations are the important new pieces.

\pyfilelines{hockey-heat.py}{31}{112}
