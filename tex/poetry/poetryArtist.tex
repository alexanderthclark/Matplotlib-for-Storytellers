...
\pyfile{thermometer.py}

\begin{center}
    \includegraphics[width = .45\textwidth]{figures/poetryplots/thermometer.pdf}
\end{center}

The two rectangles and the circle are created first, but that does not add them to the plot. The loop passes each one to \code{ax.add\_patch()}. The short horizontal line is a \code{Line2D} object, so it is added with \code{ax.add\_line()}. The call to \code{ax.text()} is a convenience: it creates a \code{Text} object and adds it to the axes in one step. We assign that object to \code{label} so that its color can be changed afterward with \code{set\_color()}.

The thermometer also shows why it helps to have access to the individual objects. The value controls the height of one rectangle and the locations of the line and label. The \code{zorder} values keep the fill above the stem and the bulb above both. We could now change or remove any part without redrawing the others.
...
Other Artist classes appear later without requiring much additional theory. \code{ax.scatter()} returns a \code{PathCollection}, while \code{imshow()} returns an \code{AxesImage}. The hockey example uses \code{OffsetImage} and \code{AnnotationBbox} to place team logos. There is no need to memorize the class hierarchy. The important point is that these parts of a plot are objects that can be stored in variables, modified, layered, or removed.
...
This is more laborious than calling \code{plot()}, but nothing fundamentally different is happening. We are still using Matplotlib's object-oriented interface, only one object at a time. The reward is flexibility: we are no longer limited to the plot produced by a single method. The activity calendar, heatmaps, directed graph, and speedometer that follow look very different, but each is built by choosing a few Artist objects and placing them on an axes object.
